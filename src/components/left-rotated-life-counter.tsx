...
export default function LeftRotatedLifeCounter({ className, heightPercentage = 100, widthPercentage = 50 }: { className?: string, heightPercentage?: number, widthPercentage?: number }) {
    ...
    useEffect(() => {
        function handleResize() {
            const newHeight = window.innerWidth * (widthPercentage / 100);
            const newWidth = window.innerHeight * (heightPercentage / 100);
            setHeight(newHeight);
            setWidth(newWidth);
        }
        handleResize();
    ...
    return (
        <div className={`absolute flex flex-col items-center justify-center rotate-90 ${className}`} style={{backgroundColor: backgroundColor, color: color, width: width, height: height, left: (height)}}>
            ...
            <Colorful style={colorMenuVisible ? {position: "fixed", visibility:"visible"} : {visibility: "hidden", position: "fixed"}} disableAlpha={true} color={color} onChange={(newColor) => {changeColor(newColor.hex)}} rotation={90}/>
            <div
